...
public class LDAgent extends AbstractNegotiationParty {
        ...
        if(time <0) {
            ...
            if (lastReceivedOffer != null
               ...
               // myLastOffer = generateRandomBid();
            		...
            		for (int i=0;i<10;i++)
            		{
            			GA.ipop[i]=GA.select()[i];
            		}
            		
            		double uti = maxOften(GA.ipop);
            		myLastOffer = generateBidWithUPDOWNUtility(uti,uti);
    ...
    public double maxOften(double a[])
    {
    		double b=0;
    		for (int i=0;i<GA.allNumber;i++)
    		{
    			if(b<=a[i])
    			{
    				b=a[i];
